    ...
    <main className="min-h-screen bg-[#f1f4f7] px-3 py-5 sm:px-6 sm:py-10">
      <div className="mx-auto w-full min-w-0 max-w-[920px]">
        <header className="mb-7 text-center sm:mb-9">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">اطلاعات خود و راهکار پژوهشی پیشنهادی را در فرم زیر وارد کنید.</p>
        </header>

        <form onSubmit={handleSubmit} className="w-full min-w-0 rounded-2xl bg-white px-5 py-7 shadow-[0_12px_45px_rgba(19,45,75,0.08)] sm:px-9 sm:py-10" aria-label="فرم ثبت راهکار پژوهشی">
          ...
          <section className="mt-10 sm:mt-12" aria-labelledby="solution-info-title">
            <SectionHeading id="solution-info-title">اطلاعات راهکار پژوهشی</SectionHeading>
            <div className="space-y-5">
              <Field id="projectTitle" label="عنوان پروژه / راهکار پژوهشی" placeholder="عنوان راهکار پیشنهادی" />
              <div><label htmlFor="solutionDescription" className="mb-2 block text-sm font-bold text-[#243f60]">شرح راهکار<span aria-hidden="true" className="mr-1 text-red-500">*</span></label><textarea id="solutionDescription" name="solutionDescription" required rows={7} placeholder="شرح مختصری از راهکار پژوهشی خود ارائه دهید" className="min-h-44 w-full resize-y rounded-lg border border-slate-200 bg-white px-4 py-3 text-base leading-8 text-slate-800 outline-none transition-colors placeholder:text-slate-400 focus:border-[#2878df] focus:ring-3 focus:ring-blue-100" /></div>
          ...
          <div className="mt-9 border-t border-slate-100 pt-7 text-center"><button type="submit" disabled={submitting} className="h-12 w-full rounded-lg bg-[#2878df] px-8 text-base font-black text-white shadow-sm hover:bg-[#2169c5] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-blue-600 disabled:cursor-not-allowed disabled:opacity-65 sm:w-auto sm:min-w-64">{submitting ? "در حال ثبت..." : "ثبت راهکار پژوهشی"}</button>{message && <p role="status" className={`mt-4 rounded-lg px-4 py-3 text-sm font-bold ${message.type === "success" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>{message.text}</p>}</div>
        </form>
      </div>
    </main>
  );
