...
const benefits = [
  ["target", "رفع اولویت‌های پژوهشی شرکت"],
  ["network", "همکاری و شبکه‌سازی با پژوهشگران"],
  ["book", "دسترسی به ظرفیت علمی"],
  ["wallet", "امکان استفاده از حمایت مالی پارک فناوری پردیس"],
] as const;
...
export default function Home() {
  return <main className="landing-shell px-3 py-4 sm:px-6 sm:py-8"><div className="mx-auto w-full min-w-0 max-w-[1040px] overflow-hidden rounded-[22px] bg-white px-5 py-9 shadow-[0_12px_45px_rgba(19,45,75,0.08)] sm:px-10 sm:py-12 lg:px-[72px]">
    ...
    <section className="funding-banner mt-14 overflow-hidden rounded-2xl px-6 py-7 text-white sm:px-8 sm:py-8" aria-label="میزان حمایت مالی"><div className="relative z-10 grid items-stretch text-center sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]"><div className="flex min-h-32 flex-col items-center justify-center px-3 py-4"><p className="text-4xl font-black text-[#ffd54a] sm:text-5xl">تا ۷۰٪</p><p className="mt-2 text-sm font-medium text-blue-50 sm:text-base">حمایت از هزینه اجرای پروژه</p></div><div className="h-px w-full self-center bg-white/20 sm:h-24 sm:w-px"/><div className="flex min-h-32 flex-col items-center justify-center px-3 py-4"><div className="mb-3 grid h-14 w-14 place-items-center rounded-full bg-white/10 text-[#ffd54a]"><Icon name="bulb" className="h-8 w-8"/></div><p className="text-2xl font-black leading-relaxed text-[#ffd54a] sm:text-3xl">تا سقف ۱۰ میلیارد ریال</p><p className="mt-2 text-sm leading-7 text-blue-50">برای پروژه‌های منتخب و مطابق ضوابط برنامه</p></div></div></section>
  </div></main>;
}
